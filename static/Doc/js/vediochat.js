...
function openChat(id) {
  ...
  if (currentSocket && currentSocket.readyState === WebSocket.OPEN) {
    ...
    socket.onmessage = function(e) {
            ...
            if (data.sdp && data.username != userProfileData.username) {
                  console.log('sdp type',data.sdp.type)
                    ...
                    if(data.sdp.type === 'decline'){
                        // declinedCall()
                        // document.getElementById('callerinfo').style.right='80%'
                        document.getElementById('callerinfo').innerText = 'Your call is rejected';
                    }
                    if(data.sdp.type === 'rejected'){
    ...
    document.getElementById('callDecline').addEventListener('click',()=>{
      console.log('click declined')
      document.getElementById('vedioAnswercall').style.display='none'
      var ring=document.getElementById('caller')
      ring.pause();
      sendSignal('sdp',{type:'decline',usernm:'unk'});
    ...
    function attenedCall(dataInfo){
      ...
      console.log('click answer 10')
      navigator.mediaDevices.getUserMedia({ video: true, audio: true })
            .then(function(stream) {
                console.log('click answer 1')
                mediaStream = stream; 
